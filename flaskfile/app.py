...
app.config['UPLOAD_FOLDER'] = '/home/muthu/flaskfile/static/uploads/'
...
class Webcam(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    web_image = db.Column(db.LargeBinary)
...
def capture_and_predict():
    ...
    if file:
        image_binary = file.read()
        new_image = Webcam(web_image=image_binary)
        db.session.add(new_image)
        db.session.commit()
        ...
        return redirect(url_for('predicted', predictions=predictions, annotated_image_path=annotated_image_path))
